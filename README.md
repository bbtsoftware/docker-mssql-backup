...
| PACK                | `tar` | Packs the output files into a single `.tar.gz`-File. Uses the `/backup_tmp` directory inside the [Microsoft SQL Server] container to temporarily store the files and moves them to `/backup` afterwards. Another supported value is `zip`.
